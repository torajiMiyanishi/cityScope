...
public class BehaviorLocator {
     ...
     * @return
     */
    public static List<TSpot> getCandidateSpots(TSpotManager spotManager){
        return spotManager.getSpots(SpotType.Poi);
    }
    /**
    ...
    public static List<TSpot> getCandidateSpots(TAgent ownerAgent, Behavior.BehaviorType behaviorType, TTime curerntTime){
        ...
        Behavior.IndustryType industryType = Behavior.INDUSTRY_ID_TO_INDUSTRY_TYPE.get(residentRole.getSyntheticPopulationData().getIndustryTypeId());
            ...
            case WORK:
                if (!isAtHome){
                    if (industryType != Behavior.IndustryType.NOT_ASSIGNED) { // 職種割り当てが存在する．
                        if (!currentPoi.getPoiData().getIndustryTypes().contains(industryType)) { // 現在地が職場でない場合．
...
//                                System.out.println(industryType + " @BehaviorLocator");
            ...
            case BOTH:
                if (!isAtHome){
                    if (!currentPoi.getPoiData().getBehaviorTypes().contains(behaviorType)){
                ...
                if(curerntTime.isLessThan(12,0,0)){ // 午前中
                    if (isAtHome){ // 自宅にいる場合
                        ...
                        if (behaviorType == Behavior.BehaviorType.COMMUTING // 通勤中で
                                && !currentPoi.getPoiData().getIndustryTypes().contains(industryType)){ // 会社にいない場合
                            candidateSpots.addAll(MapApp.getSpotsByIndustryType(industryType)); // 通勤先群
                        } else if (behaviorType == Behavior.BehaviorType.SCHOOL_COMMUTING // 通学中で
                                && !currentPoi.getPoiData().getBehaviorTypes().contains(behaviorType)) { // 学校のにいない場合
                            candidateSpots.addAll(MapApp.getSpotsByBehaviorType(behaviorType)); // 通学先群
                        }
                    }
                } else { // 午後
